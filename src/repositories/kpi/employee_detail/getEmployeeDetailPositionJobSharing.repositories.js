module.exports = {
  query: () => {
    ...
    SELECT
      ...
      tejs.job_sharing_id,
      tk.employee_number,
      tejs.employee_number as job_sharing_employee_number,
    ...
    FROM tb_employee te
      LEFT JOIN tb_employee_job_sharing tejs
          ON tejs.employee_number  = te.employee_number
      INNER JOIN tb_kpi tk
          ON tejs.job_sharing_id = tk.job_sharing_id
      INNER JOIN tb_employee_position tep
        ON tep.employee_number = tk.employee_number
        AND tep.end_date = (
          SELECT
            MAX(end_date)
          FROM tb_employee_position tep2
          WHERE tep2.employee_number = tk.employee_number
        )
        AND tep.last_updated_date = (
          SELECT
            MAX(last_updated_date)
          FROM tb_employee_position tep3
          WHERE tep3.employee_number = tk.employee_number
        )
      LEFT JOIN tb_position_v2 tpv
        ON tpv.position_id = tep.position_id
      LEFT JOIN tb_kpi_score_final tksf
        ON tksf.employee_number = tk.employee_number
        AND tksf.createdAt = (
          SELECT
            MAX(createdAt)
          FROM tb_kpi_score_final tksf2
          WHERE tksf2.employee_number = tk.employee_number
    ...
    WHERE 1=1
      ...
      AND tk.job_sharing_id IS NOT NULL
    GROUP BY
      te.employee_id,
      tk.employee_number ,
      tejs.job_sharing_id,

const { mint, check } = require("../../../src/commons/helpers/jwt");
...
module.exports = async (res, uid, isRemember, fcm_token, req, targetUID = null) => {
  // Find User
  let selectedUID = uid
  ...
  group_master = await EmployeeRepository.getGroupMasterByUserIdRepository(
    user.user_id
  );
  ...
  // GET TIER 1 KMAP GROUP ID FOR cabang, anak perusahaan, non cluster
  const typeOrgValue = group_master[0]?.type_org || group_master[0]?.org_type;
  if (
    typeOrgValue === "Cabang" ||
    typeOrgValue === "Anak Perusahaan" ||
    typeOrgValue === "Non Cluster & Afiliasi"
  ) {
    kmap_group_id = await EmployeeRepository.getTier1ForCabangAnperNonCluster(group_master[0]?.group_master_id);
  }
  ...
  const payload = {
    ...
    kmap_group_id: kmap_group_id?.[0]?.group_master_id || null,
  ...
  if (!userToken) {
    ...
    await UserToken.create({
      ...
      expired_token: check(refreshToken).exp,
    });
  } else if (
    userToken &&
